...
## What is the difference between shopping cart and catalog price rules?
  ...
  * Catalog price rule применяются к продуктам и они отображаются ещё до их добавления в корзину.
  * shopping cart rule применяется к корзине(квоте), например это могут быть правила вроде бесплатной доставки, или скидки для при покупке опреденыёх товаров и т.д.

## What are the  of Magento shopping cart rules?

  * К корзине может быть применено несколько правил, но при этом может быть применено только одно правило с промо кодом
  * Правила применяются независимо друг-от-друга

## Как применяются правила

1. При подсчёте итогов вызывается тотал `discount`
```xml
<discount>
...
```
2. В методе `Mage_SalesRule_Model_Quote_Discount->collect(Mage_Sales_Model_Quote_Address $address)` происходит цикл по всем айтемам квоты:
```php
<?
...
```
3. Таким образом каждому айтему(кроме айтемов без скидок и дочерних айтемов) применяется `$this->_calculator->process($child)`, в `_calculator` содержится экземпляр `Mage_SalesRule_Model_Validator`.
```php
<?
